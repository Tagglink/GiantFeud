...
public class Items : MonoBehaviour {
    ...
    public static Dictionary<ItemID, Item> itemList = new Dictionary<ItemID, Item> {
            ...
            { ItemID.ELIXIR,        new Consumable  ("Elixir",          "En magisk brygd.",                         new Resources(0,4,8,8,0),       null, 5.0f, new Stats(0,0,0,0,0,0), 20.0f,     (Giant giant) => { giant.AddMultiplier(ItemID.ELIXIR, new Stats(2, 2, 2, 2, 2, 2)); }, (Giant giant) => { giant.RemoveMultiplier(ItemID.ELIXIR); }) },
            ...
            { ItemID.MEATSTEW,      new Consumable  ("Köttgryta",       "Bubblande med näring.",                    new Resources(0,0,3,2,0),       null, 5.0f, new Stats(0,0,0,0,0,0), 60.0f,     (Giant giant) => { giant.stats.hp += 100; giant.AddBuff(ItemID.MEATSTEW, new Stats(2, 0, 0, 0, 0, 2)); }, (Giant giant) => { giant.RemoveBuff(ItemID.MEATSTEW); }) },
            { ItemID.MEATCLUB,      new Consumable  ("Köttklubba",      "Inte för att slåss.",                      new Resources(1,0,8,0,0),       null, 5.0f, new Stats(0,0,0,0,0,0), 10.0f,     (Giant giant) => { giant.AddBuff(ItemID.MEATCLUB, new Stats(0, 0, 0, 0, 0, 10)); }, (Giant giant) => { giant.RemoveBuff(ItemID.MEATCLUB); giant.camp.GetComponent<Camp>().resources.wood += 1; giant.stats.hp += 400; }) },
            { ItemID.BREADLOAF,     new Consumable  ("Limpa",           "Stor som ett hus.",                        new Resources(0,8,0,2,0),       null, 5.0f, new Stats(0,0,0,0,0,0), 10.0f,     (Giant giant) => { giant.stats.hp += 200; giant.AddBuff(ItemID.BREADLOAF, new Stats(0, 1, 0, 0, 0, 0)); }, (Giant giant) => { giant.RemoveBuff(ItemID.BREADLOAF); }) },
            { ItemID.ASGARDMEAL,    new Consumable  ("Asgårdmål",       "Tors favorit.",                            new Resources(0,20,20,15,0),    null, 5.0f, new Stats(0,0,0,0,0,0), 0.0f,      (Giant giant) => { giant.AddBuff(ItemID.ASGARDMEAL, new Stats(0, 0, 0, 1000, 0, 0)); giant.stats.hp = giant.stats.maxHP + 1000; }, (Giant giant) => { }) }
        };
}
